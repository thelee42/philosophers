...
/*   Updated: 2025/03/17 14:45:26 by thealee          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{    
    ...
    write_lock(philo, "is thinking");
    if ((philo->shared->num_philos % 2) != 0)
    {
        pthread_mutex_lock(&philo->shared->lock_turn);
        if (philo->shared->rest_turn == philo->name)
        {
            precise_usleep(2000);
            philo->shared->rest_turn = (philo->shared->rest_turn % philo->shared->num_philos) + 1;
            pthread_mutex_unlock(&philo->shared->lock_turn);
            return ;
        }
        pthread_mutex_unlock(&philo->shared->lock_turn);
    }
